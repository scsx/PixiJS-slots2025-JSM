      ...
      () => {
        ...
        if (forceWin) {
            ...
            for (const symbol of reel.symbols) {
                ...
                if (symbol.y + SYMBOL_SIZE / 2 > visibleAreaMinY && symbol.y - SYMBOL_SIZE / 2 < visibleAreaMaxY) {
            ...
            if (visibleSymbols.length >= NUM_VISIBLE_SYMBOLS) {
                ...
                visibleSymbols[0].texture = slotTextures[Math.floor(Math.random() * slotTextures.length)];
                visibleSymbols[2].texture = slotTextures[Math.floor(Math.random() * slotTextures.length)];
            }
        }
...
function checkWin() {
  ...
  const allMatch = actualVisibleTextures.every(texture => texture !== null && texture === firstActualTexture);
...
export async function initSlotGame(appInstance, canvasCenterInstance) {
  ...
  for (let i = 0; i < 3; i++) { // Para uma máquina de slot padrão de 3 rolos.
    ...
    for (let j = 0; j < NUM_SYMBOLS_PER_REEL_STRIP; j++) { // Adiciona mais símbolos para um loop suave.
      ...
      symbol.originalY = j * CELL_HEIGHT; // Guarda a posição Y base do símbolo na tira (usa CELL_HEIGHT)
      symbol.y = symbol.originalY + CELL_HEIGHT / 2; // Posição inicial visual (centro do símbolo na célula)
  ...
  _app.ticker.add(() => {
    for (let i = 0; i < reels.length; i++) {
      ...
      for (let j = 0; j < r.symbols.length; j++) {
        ...
        // `currentSymbolTopY` é a posição do topo da célula do símbolo.
        // Se o topo da célula está acima de -CELL_HEIGHT, significa que a célula inteira está fora do topo.
        if (currentSymbolTopY + CELL_HEIGHT < 0) { // Verifica se o fundo da célula está acima de 0
        ...
        // Esta condição deve verificar se o topo da célula está abaixo do fim da tira completa.
        else if (currentSymbolTopY > totalStripHeight) { // CORRIGIDO: Condição para reciclagem na parte inferior
            currentSymbolTopY -= totalStripHeight;
            symbol.texture = slotTextures[Math.floor(Math.random() * slotTextures.length)];
            symbol.originalY = currentSymbolTopY - r.position;
        }

        // Aplica a posição Y calculada ao símbolo, ajustando para a âncora central (0.5) dentro da sua célula.
        symbol.y = currentSymbolTopY + CELL_HEIGHT / 2; // Usa CELL_HEIGHT
      }
    }
  });
